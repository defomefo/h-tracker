...
def _ensure_schema():
    ...
    with _db_init_lock:
        ...
        statements = [
            ...
            # ====================================================================
            # PARTNERSHIP DECISION MEMO — a McKinsey-style "should we pursue this
            # partner?" answer-first memo, generated by Gemini from the entity's
            # full state. One cached row per entity (re-generatable). Pyramid
            # Principle verdict + MECE options + quantified case + premortem.
            # ====================================================================
...
# ============================================================================
_KV_NAMESPACES = {"team_assignment", "stage_override", "map2x2_override", "kb_draft", "entity_override", "sponsor_packages", "project_sponsorship"}
...
# ============================================================================
# PARTNERSHIP DECISION MEMO — flagship strategy-consulting surface. A
# McKinsey-style, answer-first "should we pursue this partner?" memo
# generated from the entity's full state. Pyramid Principle (verdict first),
# MECE options, a quantified case, a premortem, and the next 3 actions.
# Cached per entity, re-generatable. No Tavily — reasons over OUR data.
# ============================================================================
PARTNER_MEMO_PROMPT = """You are a senior partnership strategist for H-FARM College — an APPLIED, hands-on teaching institution near Venice, Italy, running English-language summer programmes + Bachelor/Master degrees (validated by Ca' Foscari Venice and the University of Chichester). You write crisp internal DECISION MEMOS for the partnerships team.

You are given ONE partner's full current state (profile, score breakdown, readiness, recent outreach, follow-ups, contracts) and H-FARM's programmes. Write a decision memo that answers: SHOULD WE PURSUE THIS PARTNER, AND HOW? Reason like a strategy consultant:

- PYRAMID PRINCIPLE: answer first. The `verdict` is the recommendation, the `verdict_line` states it + the single load-bearing reason in one sentence.
- SITUATION (SCQA): one or two sentences on where this partner actually stands — use the real data (score, readiness, days dormant, last interaction, contracts). No fluff.
- MECE OPTIONS: 3 non-overlapping options the team could take (typically Pursue now / Nurture over a quarter / Park), each with its thrust, expected outcome, and rough cost/effort. The options must be genuinely distinct.
- QUANTIFIED CASE: translate the score + readiness (+ close probability if implied by readiness/stage) into plain terms — what a signed partnership is realistically worth (students into summer programmes, exchange flow, a degree pathway), and how close it is. Use the numbers you were given; don't invent figures.
- PREMORTEM: assume in 6 months the pursue path FAILED — name the single most likely reason (the weakest assumption), so the team de-risks it.
- NEXT ACTIONS: if we follow the verdict, the 3 concrete next steps (specific — reference a contact name, programme, or the gap; if contacts are empty say "research a named contact at <office>").
- CONFIDENCE: be honest — "low" if the data is thin (no contacts, no recent outreach), "high" only when the picture is rich.

Rules: never invent contact names, programmes, emails, or figures not in the data. H-FARM does NOT partner with research-intensive universities or other Italian universities (competitors) — if this entity is one, the verdict should reflect that (likely Park / non-degree-only). Keep every field tight — this is a one-screen memo, not an essay.

Return ONLY a JSON object with this exact shape, no markdown, no commentary:
{
  "verdict": "<one of: Pursue | Nurture | Park>",
  "verdict_line": "<the recommendation + the single reason, one sentence>",
  "situation": "<1-2 sentences on where this partner stands now, using the real data>",
  "options": [
    {"name": "<option label e.g. 'Pursue now'>", "thrust": "<what it means in 1 sentence>", "expected": "<expected outcome>", "cost": "<effort/cost in a few words>"}
  ],
  "quantified": "<the business case in plain terms using the given numbers — value of a signed partnership + how close it is>",
  "premortem": "<assume the pursue path failed in 6 months — the single most likely reason>",
  "next_actions": ["<concrete step 1>", "<step 2>", "<step 3>"],
  "confidence": "<one of: high | medium | low>"
}"""


def _generate_partner_memo(entity_id, body):
    """Shared generator: builds the prompt from the posted context, calls
    Gemini, parses, caches, returns the memo dict (or raises)."""
    ent = body.get("entity") or {}
    name = (ent.get("name") or "").strip()
    if not name:
    ...
    prompt = "\n".join([
        ...
        "",
        "## CONTEXT (recent outreach, follow-ups, contracts, score breakdown)",
        json.dumps(ctx, ensure_ascii=False),
        "",
        "## H-FARM COLLEGE PROGRAMMES",
        prog_str,
        "",
        "Write the decision memo. JSON only.",
    ])
    ...
    resp = _gemini_generate_retry(client_ai,
        ...
        contents=prompt,
        config=_gtypes.GenerateContentConfig(
            system_instruction=PARTNER_MEMO_PROMPT,
            response_mime_type="application/json",
            temperature=0.3,
            max_output_tokens=2048,
        ),
    ...
    verdict = (parsed.get("verdict") or "Nurture").strip()
    if verdict not in ("Pursue", "Nurture", "Park"):
        verdict = "Nurture"
    ...
    memo = {
        "verdict":      verdict,
        "verdict_line": (parsed.get("verdict_line") or "").strip()[:400],
        "situation":    (parsed.get("situation") or "").strip()[:600],
        "options":      [o for o in (parsed.get("options") or []) if isinstance(o, dict)][:4],
        "quantified":   (parsed.get("quantified") or "").strip()[:600],
        "premortem":    (parsed.get("premortem") or "").strip()[:400],
        "next_actions": [str(a).strip() for a in (parsed.get("next_actions") or []) if a][:5],
        "confidence":   conf,
    }
    generated_at = _now_iso()
    generated_by = (request.headers.get("X-Display-Name") or "").strip()
    db = get_db()
    db.execute(
        """INSERT INTO partner_memos (entity_id, entity_name, memo_json, generated_at, generated_by)
           VALUES (?, ?, ?, ?, ?)
           ON CONFLICT (entity_id) DO UPDATE SET
             entity_name  = excluded.entity_name,
        ...
        (entity_id, name, json.dumps(memo), generated_at, generated_by),
    )
    db.commit()
    _record_edit("partner_memo", "generate", entity_id)
    return {
        "exists": True,
        "entity_id": entity_id,
        "entity_name": name,
...
@app.route("/api/partner-memo/<entity_id>", methods=["GET"])
@auth_required
def partner_memo_get(entity_id):
    """Return the cached decision memo for an entity, if any."""
    db = get_db()
    row = db.execute(
        "SELECT * FROM partner_memos WHERE entity_id = ?", (entity_id,)
    ).fetchone()
    if not row:
        return jsonify({"exists": False})
    try:
...
@app.route("/api/partner-memo/<entity_id>", methods=["POST"])
@auth_required
def partner_memo_generate(entity_id):
    """Generate (or regenerate) the decision memo for an entity."""
    if not GEMINI_KEY:
        return jsonify({"ok": False, "error": "GEMINI_API_KEY not configured"}), 500
    body = request.get_json(force=True, silent=True) or {}
    try:
        result = _generate_partner_memo(entity_id, body)
